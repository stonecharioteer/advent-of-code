...
pub fn solve(input: AocInput) -> Result<(), AocErrors> {
    println!("Solving for 2015 day 01");
    let mut stack: Vec<char> = vec![];
    let mut depth: isize = 0;
    let mut position: Option<usize> = None;

    for (ix, paren) in input.chars().enumerate() {
        match paren {
            '(' => {
                depth += 1;
            }
            ...
            c => {
                return Err(AocErrors::InvalidInputFile(
                    format!("2015x01's input is only expected to contain either '(' or ')'. '{c}' is an unrecognized input.")));
            }
        }

        if position == None && depth < 0 {
            position = Some(ix + 1);
        }
    }
