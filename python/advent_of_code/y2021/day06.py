...
def lanternfish(data) -> Tuple[int, int]:
    """Returns the solution for 2021 day 6"""
    data = data.strip().split(",")
    data = [int(x) for x in data]
    # calculate how many lantern fish you'd have in 80 days
    part_1 = count_lanternfish(data, 80)
    part_2 = count_lanternfish(data, 256)
    return (part_1, part_2)

def count_lanternfish(data, days):
    """calculates the number of lantern fish alive at the end of X days
    See `this reddit comment for the solution <https://np.reddit.com/r/adventofcode/comments/r9z49j/comment/hngtzm4/?utm_source=share&utm_medium=web2x&context=3>`_

    Or, `this github solution <https://github.com/genius487/AdventOfCode2021/blob/main/day06/p21d06.py>`_
    """
    from copy import deepcopy
    state = dict((ix, len([x for x in data if x == ix])) for ix in range(9))
    new_state = deepcopy(state)
    num_fish = len(data)
    for _ in range(days):
        for ix in range(8, -1, -1):
            # only act if the number of fish with this timer value is non-zero
            # for the case where the timer is zero,
            # the lanternfish will give birth now
            if ix == 0:
                # each fish gives birth to a new one, with the new fish
                # having a timer of 8
                # need to add the items at state[7] because we're looping in reverse,
                # and these would have been overwritten already.
                new_state[6] = state[ix] + state[7]
                new_state[8] = state[ix]
                # the "mothers" have a timer of 6
                num_fish += state[ix]
            else:
                new_state[ix-1] = state[ix]
