...
def syntax_scoring(data):
    """Returns the syntax score"""
    ...
    for line in data:
        ...
        for character in line:
            if character in "[(<{":
                ...
                if character != matching[last_character]:
                    total_points+=syntax_score_table[character]
                    is_corrupt = True
        if not is_corrupt:
            if len(stack) > 0:
                ...
                while len(stack)>0:
                    character = stack.pop()
                    if character in "[(<{":
                        required_character = matching[character]
                        autocomplete_score *= 5
                        autocomplete_score += autocomplete_scoring[required_character]
                autocomplete_scores.append(autocomplete_score)
    part_1 = total_points
    autocomplete_scores.sort()
    part_2 = autocomplete_scores[len(autocomplete_scores)//2]
