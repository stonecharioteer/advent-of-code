...
def dumb_octopus(data):
    """Solves the dumb octopus problem"""
    # isn't this another breadth-first algorithm?
...
# copied over code from https://github.com/DenverCoder1/Advent-of-Code-2021/blob/main/Day-11/part1.py
...
class Point:
    ...
    def __repr__(self) -> str:
        ...
        return f"Point(({self.x},{self.y}), level: {self.level})"
...
class Grid:
    ...
    def neighbors(self, point: Point) -> list[Point]:
        """Returns a list of all points that are neighbors of
        a given point"""
        x, y = point.x, point.y
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if (
                    ...
                    continue
                neighbors.append(self.points[y+i][x+j])
        return neighbors

    def increment_all_levels(self):
        """Increments the level of all points by 1"""
        for row in self.points:
            for point in row:
                point.level += 1

    def flash_points(self) -> int:
        """Flashes all the points that have reached the maximum level, increasing the neighbors' levels by 1"""
        flashed = 0
        for row in self.points:
            for point in row:
                if self.max_level < point.level < float("inf"):
                    # for each point that reaches the max level, set the level to infinity.
    ...
    def reset_flashed_points(self):
        """Resets flashed points to 0"""
        for row in self.points:
            for point in row:
    ...
    def advance(self, steps: int=1) -> int:
        ...
        * During a single step, each point's level is raised by 1.
        * Every point that reaches self.max_level is flashed.
        * A single point can be flashed only once per step.
        * All points that are flashed are reset
        """
        total_flashed = 0 
        for _ in range(steps):
            # increment all levels by 1
            self.increment_all_levels()
            # flash all the points
            while flash_count := self.flash_points():
    ...
    def __repr__(self) -> str:
        return "\n".join(
                [
                    "".join(
                        [
                            (str(self.points[row][col].level)).rjust(3)
