...
same width, so they only have to worry about the length they need to order,
...
def run(inp: TextIO) -> Tuple[int, int]:
    ...
    for line in inp:
        ...
        areas = (l*w, w*h, h*l)
        area = sum(areas)*2 + min(areas)
        total_area += area

        perimeter = (l+w, w+h, h+l)
        total_ribbon_length += (min(perimeter)*2 + l*w*h)
        #3818626
